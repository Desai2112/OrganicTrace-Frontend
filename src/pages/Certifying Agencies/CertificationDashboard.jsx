...
export default function CertificationDashboard() {
  ...
  const quickStats = [
    { label: 'Pending Requests', value: '24', change: '+8%', icon: <Clock className="w-5 h-5" /> },
    { label: 'Active Certifications', value: '156', change: '+12%', icon: <CheckCircle2 className="w-5 h-5" /> },
    { label: 'Scheduled Audits', value: '18', change: '+5%', icon: <Calendar className="w-5 h-5" /> },
    { label: 'Certified Entities', value: '89', change: '+15%', icon: <Users className="w-5 h-5" /> }
  ];
          ...
          <div className="bg-white/80 backdrop-blur-md rounded-xl shadow-lg p-6 border border-white/20">
            ...
            <div className="space-y-4">
              {recentRequests.map((request) => (
                <div key={request.id} className="flex items-center justify-between p-4 bg-gray-50/80 rounded-lg">
                  <div className="flex items-center space-x-4">
                    <div className={`w-2 h-2 rounded-full ${statusStyles[request.status].replace('text', 'bg')}`} />
                    <div>
              ...
              {upcomingAudits.map((audit, index) => (
                <div key={index} className="p-4 bg-gray-50/80 rounded-lg">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{audit.company}</p>
